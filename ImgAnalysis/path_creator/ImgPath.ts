...
export class EulerImg {
    ...
    constructor(img_path: string, sigmaForGuassianBlur?, root?: HTMLElement) {
        this.img = new Image();
    ...
    // // ATTEMPT AT TENSORFLOW OPTIMIZATION FOR SIMILARITY SCORING
    // optimizeTransformation_WORK_IN_PROGRESS(keyPointsToMap: Array<Array<number>>) {
    ...
    //         const cosTheta: tf.Scalar = tf.cos(theta);
    //         var cosTheta_val = 0
    //         cosTheta.data().then(value => {cosTheta_val = value[0]})
    //         const sinTheta: tf.Scalar = tf.sin(theta);
    //         var sinTheta_val = 0
    //         sinTheta.data().then(value => {sinTheta_val = value[0]})
    //         const rotation = tf.tensor([
    //             [cosTheta_val, -sinTheta_val],
    //             [sinTheta_val, cosTheta_val]
    //         ]);
    //         // console.log("X: " + X.shape)
    //         // console.log("rotation: " + rotation.shape)
    //         const scaledRotated = tf.matMul(X, rotation).mul(scale); 
    //         // console.log("scaledRotation: " + scaledRotated.shape)
    //         const tiledTranslation = translation.tile([X.shape[0], 1]); 
    //         // console.log("translation: " + tiledTranslation.shape)
    //         const X_prime = scaledRotated.add(tiledTranslation); 
    //         // console.log("X': " + X_prime.shape)
    //         return X_prime
    //     };
    ...
    //     const params = [scale.dataSync(), theta.dataSync(), translation.dataSync()]
...
export function getSimilarityScore(X: Array<Array<number>>, basepoints: Array<Array<number>>) {
...
function hausdorffDistance(setA: Array<Array<number>>, setB: Array<Array<number>>) {
    function directedHausdorff(set1: Array<Array<number>>, set2: Array<Array<number>>) {
      let maxDist = 0;
      for (let pointA of set1) {
        let minDist = Infinity;
        for (let pointB of set2) {
          let dist = Math.sqrt((pointA[0] - pointB[0]) ** 2 + (pointA[1] - pointB[1]) ** 2); 
