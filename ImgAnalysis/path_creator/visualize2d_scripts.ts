...
// import { makeMovable } from "./feature_scripts/ui.js"
import { matchesGlob } from "path"
import { isMethodSignature, Type, updateSourceFile } from "typescript"
import { serialize } from "v8"
import { maxHeaderSize } from "http"

export const root = document.getElementById("root")
export var workspace: HTMLElement

export function SetWorkspace() {
    var workspace_element = document.createElement("div")
    workspace_element.style.height = "100%"
    workspace_element.style.width = "100%"
    ...
    } else {throw new Error("Workspace is too small (min 350x350px)")}
}
...
export class Runtime {
    ...
    // animations
    time = this.vanillaVariableTracker(0) 
    ...
    CreateTicker(TickerFunction) {
        ...
        var ticker_interval_input = document.getElementById("tickerInterval") as HTMLInputElement
...
export class Ellipse {
    ...
    constructor(x: number, y: number, r?: number, w?: number, h?: number) {
        if (workspace == undefined) {throw new Error("Workspace is not defined!")}
    ...
    Fill(color, opacity?: number) {
        this.ellipse.style.backgroundColor = color
        if (opacity) this.ellipse.style.opacity = opacity.toString()
        return this
    }

    Stroke(color, hideBool: boolean) {
        this.ellipse.style.borderColor = color
        if (hideBool==true) {this.ellipse.style.border = "rgba(0,0,0,0)"}
        return this
    }
...
export class Line {
    line: HTMLElement
    start_coords: Array<number>
    end_coords: Array<number>
    ...
    constructor(x1, y1, x2, y2, width) {
        ...
        var mag = Math.sqrt((x2 - x1)**2 + (y2 - y1)**2)
    ...
    Stroke(color, opacity?) {
        ...
        if (opacity) this.line.style.opacity = opacity.toString()
        return this
    }
    Draggable() {
    ...
    AddTickmarks(tick_len, labels?:boolean, min?, max?, y_spacing=30, x_spacing=10) {
...
export class Rectangle {
    ...
    Fill(color, opacity?: number) {
        ...
        if (opacity) this.rect.style.opacity = opacity.toString()
        return this
    }

    Stroke(showStroke: boolean, color?, width?) {
...
export class UnitVector {
    ...
    constructor(x1, y1, x2, y2, color="white") {
        ...
        var mag = Math.sqrt( (x2 - x1)**2 + (y2 - y1)**2 )
        var arrowhead_hypo = Math.sqrt((0.2 * mag)**2 + (0.05 * mag)**2)
...
export class Graph {
    ...
    dims: Array<number>
    pos: Array<number>
    x_range: Array<number> =  [0,10]
    y_range: Array<number> =  [0,10]

    constructor(x:number, y:number, width:number, height:number, background=true) {
        [this.dims, this.pos] = [[width, height], [x,y]]
        const padding = 10

        if (background) {const fig = new Rectangle(x + 35/2 - 30, y + 10, width + 2*padding + 35, height + 2*padding + 20).Fill("black").Stroke(false)}
        ...
        const varListener = {
            set: (target, prop, value, receiver) => {
                ...
                if(prop == "points") {this.UpdatePoints(value)}
                else if (prop == "lines")  {this.UpdateLines(value)}
                else if (prop == "vectors") {this.UpdateVectors(value)}
                else if (prop == "functions") {this.UpdateFunctions(value)}
                else {throw new Error("property not recognized")}
    ...
    PltPosToPxPos(pos, axis) {
        ...
        } else {throw new Error("PltPosToPxPos: invalid axis arg")}
    }

    // GraphData functions

    UpdateGraph(points, lines, vectors) {
        var combined_points = points.map((point: Array<any>) => {return (point.slice(0,2))}) 
        var combined_lines = []
        lines.map((line: Array<any>) => {line.slice(0,2).forEach((element)=>{combined_lines.push(element)})})
        console.log("combined lines")
        console.log(combined_lines)
        var combined_vectors = []
        vectors.map((vect: Array<Array<number>>) => {vect.slice(0,2).forEach((element)=>{combined_lines.push(element)})})
    ...
    UpdatePoints(points: Array<Array<any>>) {
        // 0=x, 1=y, 2=s, 3=c
    ...
    UpdateLines(lines) {
        ...
        for (var i=0; i<lines.length; i++) {
            ...
            var data_line = new Line(
                this.PltPosToPxPos(line[0][0], 'x'), 
                this.PltPosToPxPos(line[0][1], 'y'),
                this.PltPosToPxPos(line[1][0], 'x'),
                this.PltPosToPxPos(line[1][1], 'y'),
                line[2]
            )
    ...
    Scatter(X: Array<number>, Y: Array<number>, s?, c?) {
        if (X.length == Y.length) {null} else {throw new Error("X and Y arrays are different lengths")}
        if (s == undefined) {s = new Array(X.length).fill(2)} else {typeof s === "number" ? s = new Array(X.length).fill(s) : null} // size param regularization (makes array)
        if (X.length == s.length) {null} else {throw new Error("X/Y and size arrays are different lengths")}
        if (c == undefined) {c = new Array(X.length).fill("white")} else {typeof c === "string" || typeof c[0] === "number" ? c = new Array(X.length).fill(c) : null} // color param regularization (makes array)
        if (X.length == c.length) {null} else {throw new Error("X/Y and color arrays are different lengths")}
    ...
    Plot(X, Y, width, grpahPoints=false) {
        ...
        var lines = []
        for (var i=0; i<X.length;i++) {lines.push([X[i], Y[i]])}
...
export class VectorField {
    ...
    constructor(x, y, width, height, dxdt: Function, dydt: Function) {
        ...
        const varListener = {
            set: (target, prop, value, receiver) => {
                ...
                this.graph.UpdateAxes(
                    [this.x_range[0] - x_padding, this.x_range[1] + x_padding],
                    [this.y_range[0] - y_padding, this.y_range[1] + y_padding],
                )
...
function vector_heater(mag, min_mag, max_mag) {
    ...
    const sinfunc = (M):any => {0.5 * Math.sin((Math.PI / 3) * i + M)}
    const M_arr = [Math.PI/2 - 0.6, -0.6, -Math.PI/2-0.6 ]
    var color_addon: Array<number> = M_arr.map(sinfunc)
    var rgb_arr = [] 
    for (let i = 0; i < 3; i++) {
...
function magnitude(vector) {return Math.sqrt(vector.x ** 2 + vector.y ** 2)}
